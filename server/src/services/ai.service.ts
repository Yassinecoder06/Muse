...
export const ai = {
  ...
  rewrite: (content: string, mode: string) => chat([{ role: 'system', content: `Rewrite the text in ${mode} mode. Return only the rewritten text. Preserve meaning. Images inside the note are marked as [IMAGE n], each standing exactly where an image belongs. Keep every [IMAGE n] token exactly where it is: never move, reorder, reword, translate, merge, or drop a marker, and never change its number. The number of markers in your output must equal the number in the input. Rewrite only the words around the markers.` }, { role: 'user', content }]),
