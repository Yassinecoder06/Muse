import { Archive, Check, FileText, FolderOpen, Moon, PanelLeft, PanelLeftClose, Plus, RotateCcw, Search, Settings, Sparkles, Star, Sun, Trash2 } from 'lucide-react'
...
export function Dashboard({ notes, counts, scope, setScope, query, setQuery, create, open, dark, toggleTheme, ask, collections, settings, userEmail, userName, emptyTrash, deleteSelected, restoreSelected }: { notes: Note[]; counts: Counts; scope: string; setScope: (value: string) => void; query: string; setQuery: (value: string) => void; create: () => void; open: (note: Note) => void; dark: boolean; toggleTheme: () => void; ask: () => void; collections: () => void; settings: () => void; userEmail: string; userName: string; emptyTrash: () => void; deleteSelected: (ids: string[]) => void; restoreSelected: (ids: string[]) => void }) {
  ...
  const [collapsed, setCollapsed] = useState(() => localStorage.getItem('muse-sidebar') === 'collapsed')
  ...
  const toggleSidebar = () => setCollapsed(current => { localStorage.setItem('muse-sidebar', current ? 'expanded' : 'collapsed'); return !current })

  return <div className={`app-layout${collapsed ? ' sidebar-collapsed' : ''}`}>
    <aside className="sidebar">
      <div className="brand"><i><span/></i> Muse<button className="sidebar-toggle" onClick={toggleSidebar} title={collapsed ? 'Expand sidebar' : 'Collapse sidebar'}>{collapsed ? <PanelLeft size={18}/> : <PanelLeftClose size={18}/>}</button></div>
      <button className="new-note" onClick={create} title="New note"><Plus size={17}/>New note <kbd>⌘ N</kbd></button>
      <nav>{nav.map(item => { const Icon = item.icon; return <button key={item.id} title={item.name} className={scope === item.id ? 'active' : ''} onClick={() => setScope(item.id)}><Icon size={17}/><span>{item.name}</span><em>{counts[item.id as keyof Counts]}</em></button> })}</nav>
      <div className="sidebar-footer"><button title="Collections" onClick={collections}><FolderOpen size={17}/><span> Collections</span></button><button title="Settings" onClick={settings}><Settings size={17}/><span> Settings</span></button><div className="user"><div>{initials}</div><span><strong>{displayName}</strong><small>{userEmail || 'Personal space'}</small></span></div></div>
    </aside>
    <main className="dashboard-main">
      <header className="topbar"><button className="sidebar-collapse" onClick={toggleSidebar} title={collapsed ? 'Expand sidebar' : 'Collapse sidebar'}><PanelLeft size={18}/></button><div className="mobile-logo">Muse</div><div className="search-field"><Search size={18}/><input value={query} onChange={e => setQuery(e.target.value)} placeholder="Search your notes"/><kbd>⌘ K</kbd></div><button className="theme" onClick={toggleTheme}>{dark ? <Sun size={18}/> : <Moon size={18}/>}</button><button className="new-note top-new" onClick={create}><Plus size={17}/> <span>New note</span></button></header>
