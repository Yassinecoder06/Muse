...
export function Dashboard({ notes, counts, scope, setScope, query, setQuery, create, open, dark, toggleTheme, ask, collections, settings }: { notes: Note[]; counts: Counts; scope: string; setScope: (value: string) => void; query: string; setQuery: (value: string) => void; create: () => void; open: (note: Note) => void; dark: boolean; toggleTheme: () => void; ask: () => void; collections: () => void; settings: () => void }) {
  ...
  return <div className="app-layout">
    <aside className="sidebar">
      ...
      <div className="sidebar-footer"><button onClick={collections}><FolderOpen size={17}/> Collections</button><button onClick={settings}><Settings size={17}/> Settings</button><div className="user"><div>JM</div><span><strong>Jordan Miller</strong><small>Personal space</small></span></div></div>
    </aside>
    <main className="dashboard-main">
      ...
      <section className="dashboard-content">
        <div className="hero"><div><p>FRIDAY, JULY 19</p><h1>Good morning, Jordan.</h1><span>A quiet space for your thoughts.</span></div><button className="ask-muse" onClick={ask}><Sparkles size={17}/> Ask Muse</button></div>
